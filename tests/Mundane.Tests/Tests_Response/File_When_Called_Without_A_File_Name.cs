...
public static class File_When_Called_Without_A_File_Name
{
	...
	public static async Task Adds_The_Content_Type_Header_For_The_Value_Passed_To_It()
	{
		...
		var response = await MundaneEngine.ExecuteRequest(
			MundaneEndpointFactory.Create(() => Response.File(_ => ValueTask.CompletedTask, contentType)),
	...
	public static async Task Sets_The_BodyWriter_To_The_Value_Passed_To_It()
	{
		...
		var response = await MundaneEngine.ExecuteRequest(
			MundaneEndpointFactory.Create(() => Response.File(o => o.Write(output), Guid.NewGuid().ToString())),
			RequestHelper.Request());
	...
	{
		var response = await MundaneEngine.ExecuteRequest(
			MundaneEndpointFactory.Create(() => Response.File(_ => ValueTask.CompletedTask, Guid.NewGuid().ToString())),
			RequestHelper.Request());
